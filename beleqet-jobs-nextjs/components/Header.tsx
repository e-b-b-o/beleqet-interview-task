    ...
    <header className="sticky top-0 z-50 bg-white/90 backdrop-blur border-b border-border">
      <div className="container-page flex items-center justify-between h-16">
        ...
        <div className="flex items-center gap-3">
          {user ? (
            <>
              <span className="text-sm text-ink font-semibold">Hi, {user.firstName}</span>
              <button
                onClick={logout}
                className="hidden sm:inline-block text-sm font-medium text-muted hover:text-red-500 transition-colors"
              >
                Logout
              </button>
            </>
